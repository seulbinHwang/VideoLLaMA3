...
def get_mm_adapter_state_maybe_zero_3(named_params, keys_to_match):
    to_return = {
        k: t for k, t in named_params if any(key_match in k
...
def get_modality_length_grouped_indices(lengths,
                                        batch_size,
                                        world_size,
                                        generator=None):
    ...
    if all(l > 0 for l in lengths) or all(l < 0 for l in lengths):
        ...
        return get_length_grouped_indices(lengths,
                                          batch_size,
                                          world_size,
                                          generator=generator)
    ...
    mm_shuffle = [
        mm_indices[i] for i in get_length_grouped_indices(
            mm_lengths, batch_size, world_size, generator=None)
    ]
    lang_shuffle = [
        lang_indices[i] for i in get_length_grouped_indices(
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.llm_lr is not None:
                lm_parameters = [
                    name for name, _ in optimized_parameters if
                    "vision_encoder" not in name and "mm_projector" not in name
                ]
                decay_lm_parameters = [
                    name for name in lm_parameters if name in decay_parameters
                ]
                nodecay_lm_parameters = [
                    name for name in lm_parameters
                    if name not in decay_parameters
                ]
                optimizer_grouped_parameters.extend([{
                    "params": [
                        p for n, p in optimized_parameters
                        if n in decay_lm_parameters
                    ],
            ...
            if self.args.mm_projector_lr is not None:
                ...
                decay_projector_parameters = [
                    name for name in projector_parameters
                    if name in decay_parameters
                ]
                nodecay_projector_parameters = [
                    name for name in projector_parameters
                    if name not in decay_parameters
                ]
                optimizer_grouped_parameters.extend([{
                    "params": [
                        p for n, p in optimized_parameters
                        if n in decay_projector_parameters
                    ],
                    "weight_decay": self.args.weight_decay,
                    ...
                    "params": [
                        p for n, p in optimized_parameters
                        if n in nodecay_projector_parameters
                    ],
                    "weight_decay": 0.0,
                    "lr": self.args.mm_projector_lr,
                }])
            ...
            if self.args.vision_encoder_lr is not None:
                ...
                decay_vision_encoder_parameters = [
                    name for name in vision_encoder_parameters
                    if name in decay_parameters
                ]
                nodecay_vision_encoder_parameters = [
                    name for name in vision_encoder_parameters
                    if name not in decay_parameters
                ]
                optimizer_grouped_parameters.extend([{
                    "params": [
                        p for n, p in optimized_parameters
                        if n in decay_vision_encoder_parameters
                    ],
                    "weight_decay": self.args.weight_decay,
                    ...
                    "params": [
                        p for n, p in optimized_parameters
                        if n in nodecay_vision_encoder_parameters
                    ],
                    "weight_decay": 0.0,
                    "lr": self.args.vision_encoder_lr,
                }])
