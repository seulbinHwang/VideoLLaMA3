...
def load_mm_projector(model_path, cache_dir=None, token=None):
    if os.path.exists(os.path.join(model_path, 'mm_projector.bin')):
        ...
        is_local = False
        folder = parse_snapshot_folder(model_path,
                                       cache_dir=cache_dir,
                                       repo_type="model")
    ...
    mm_projector_weights = torch.load(os.path.join(folder, 'mm_projector.bin'),
                                      map_location='cpu')
...
class SimSpatialConv(nn.Module):
    ...
    def __init__(self,
        ...
        self.readout = build_mlp(mlp_depth, 4 * self.encoder_hidden_size,
