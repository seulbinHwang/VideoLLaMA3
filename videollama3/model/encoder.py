...
class CLIPVisionEncoder(nn.Module):
    ...
    def __init__(self, vision_encoder, args, delay_load=False):
        ...
        if not delay_load:
            self.attn_implementation = getattr(args, 'mm_attn_implementation',
                                               'flash_attention_2')
            self.load_model()
        else:
    ...
    def load_model(self):
        if self.is_loaded:
            print(
                'Vision tower is already loaded, `load model` call again, skipping.'
            )
            return
    ...
    def forward(self, images, **kwargs):
        ...
        if type(images) is list:
            image_features = []
            for image in images:
                image_forward_out = self.vision_encoder(
            ...
            image_forward_outs = self.vision_encoder(images.to(
...
class Videollama3VisionEncoder(nn.Module):
    ...
    def __init__(self, vision_encoder, args, delay_load=False):
        ...
        if not delay_load:
            ...
            # uncertain whether flash-attention-2 is supported during inference phase.
            self.attn_implementation = 'sdpa'  # 'eager'
            self.cfg_only = Videollama3VisionEncoderConfig.from_pretrained(
                self.vision_encoder_name)
    ...
    def load_model(self, args):
        ...
        self.image_processor = Videollama3ImageProcessor.from_pretrained(
            self.vision_encoder_name)
    ...
    def forward(self, pixel_values, grid_sizes, merge_sizes, **kwargs):
        image_features = self.vision_encoder(pixel_values, grid_sizes,
                                             merge_sizes)
        return image_features
...
def build_vision_encoder(vision_encoder_cfg, **kwargs):
    vision_encoder = getattr(
        vision_encoder_cfg, 'mm_vision_encoder',
        getattr(vision_encoder_cfg, 'vision_encoder', None))

    if 'clip' in vision_encoder:
        vision_encoder = CLIPVisionEncoder(vision_encoder,
                                           args=vision_encoder_cfg,
                                           **kwargs)
    elif 'siglip' in vision_encoder:
        vision_encoder = SiglipVisionEncoder(vision_encoder,
                                             args=vision_encoder_cfg,
                                             **kwargs)
    elif 'navit' in vision_encoder.lower():
        vision_encoder = Videollama3VisionEncoder(vision_encoder,
                                                  args=vision_encoder_cfg,
                                                  **kwargs)
    else:
