...
def loadFile(name):
    ...
    # load file chunks if too big
        ...
        data = {}
        chunks = glob.glob('{dir}/{dir}_*.{ext}'.format(dir=name.split(".")[0],
                                                        ext=name.split(".")[1]))
        for chunk in chunks:
...
def getStepsNum(question):
    return len([
        c for c in question["semantic"] if not (any([
...
def intsFromSlice(strSlice):
    ...
    return (range(slice_obj.start or 0, slice_obj.stop or -1, slice_obj.step or
                  1))
...
def updateConsistency(questionId, question, questions):
    inferredQuestions = [
        eid for eid in question["entailed"] if eid != questionId
    ]
...
def computeGroundingScore(question, sceneGraph, attentionMap):
    ...
    if args.objectFeatures:
        cells = [
            ((x0, y0, x1, y1), attention) for x0, y0, x1, y1, attention in cells
        ]
    else:
        cells = [(getCell(i, j), attentionMap[i][j])
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=
        "True to compute consistency score (Need to provide answers to questions in val_all_questions.json)."
    )
    parser.add_argument(
        '--grounding',
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=int,
        help=
        "Optional, only to get attention score. Images features map size, mapSize * mapSize"
    )
    ...
    questions = loadFile(
        f'{args.data_folder}/{args.questions.format(tier = args.tier)}')
    ...
    for qid in questions:
        if (qid not in predictions) and (args.consistency or
                                         questions[qid]["isBalanced"]):
            print(
                "no prediction for question {}. Please add prediction for all questions."
                .format(qid))
    ...
    if args.grounding:
        with open(
                f'{args.data_folder}/{args.attentions.format(tier = args.tier)}'
        ) as attentionsFile:
    ...
    scores = {
        "accuracy": [
        ],  # list of accuracies per question (1 if correct else 0). Will be averaged ultimately.
        "binary": [
        ],  # list of accuracies per a binary question (1 if correct else 0). Will be averaged ultimately.
        "open": [
        ],  # list of accuracies per an open question (1 if correct else 0). Will be averaged ultimately.
        "validity": [],  # list of validity per question (1 if valid else 0).
        "plausibility": [
        ],  # list of plausibility per question (1 if plausible else 0).
        ...
        "accuracyPerLength": defaultdict(
            list),  # list of question accuracies per question's word number.
    ...
    for qid, question in tqdm(questions.items()):
        ...
        if question["isBalanced"]:
            ...
            answerType = "open" if question["types"][
                "structural"] == "query" else "binary"
    ...
    metrics = [
        "binary", "open", "accuracy", "consistency", "validity", "plausibility",
