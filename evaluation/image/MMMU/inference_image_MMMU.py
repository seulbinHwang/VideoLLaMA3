...
class MMMUDataset(Dataset):
    ...
    def __init__(self,
        ...
        if num_chunks > 1:
            self.index_list = get_chunk(list(range(len(self.dataset))),
    ...
    def process_single_sample(self, data):
        ...
        if self.interleaved_format:
            ...
            return_image_list = [
                data[image_path] for image_path in return_image_list
            ]

            return_options = data['options']
            if len(o_imgs_paths) > 0:
                for idx in range(1, 8):
                    return_options = return_options.replace(
                        f'<image {idx}>', '<image>')
    ...
    def construct_prompt(self, sample, config):
        ...
        if sample['question_type'] == 'multiple-choice':
            ...
            empty_prompt_sample_structure = config[
                'multi_choice_example_format']
            empty_prompt = empty_prompt_sample_structure.format(
                question, example)
            res_dict = {}
            ...
            if config['task_instructions']:
                res_dict['final_input_prompt'] = config[
                    'task_instructions'].strip() + '\n\n' + empty_prompt
            else:
                res_dict['final_input_prompt'] = empty_prompt

            res_dict['gt_content'] = options[ord(sample['answer'].upper()) -
                                             ord('A')]
        else:
...
def run_inference(args):
    ...
    dataset = MMMUDataset(args.data_path,
                          processor["image"],
                          args.split,
                          config,
                          args.num_chunks,
                          args.chunk_idx,
                          interleaved_format=args.interleaved)
    ...
    for idx, sample in enumerate(tqdm(dataloader)):
        ...
        question = sample[0][
            'final_input_prompt'] if args.instructions_message == '' else f'{sample[0]["final_input_prompt"]} {args.instructions_message}'
        ...
        if question_type == 'multiple-choice':
            ...
            pred_ans = parse_multi_choice_response(output, all_choices,
                                                   index2ans)
        else:  # open question
            pred_ans = output
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config-path',
                        type=str,
                        default="./evaluation/image/MMMU/config_file.yaml")
