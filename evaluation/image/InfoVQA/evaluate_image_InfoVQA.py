...
answer_types = {
    ...
    'multiple spans': 'Multi-Span',
    'non span': 'None span'
}
evidence_types = {
    'table/list': 'Table/list',
    'textual': 'Text',
    'photo/pciture/visual_objects': 'Visual/Layout',
    'figure': 'Figure',
    'map': 'Map'
}
...
def validate_data(gtFilePath, submFilePath, num_chunks):
    ...
    if num_chunks > 1:
        ...
        for _idx in range(args.num_chunks):
            file = submFilePath.replace('.jsonl',
                                        f'_{args.num_chunks}_{_idx}.jsonl')
    ...
    if isinstance(submJson, list) == False:
        raise Exception(
            "The Det file is not valid (root item must be an array)")

    if len(submJson) != len(gtJson['data']):
        raise Exception(
            "The Det file is not valid (invalid number of answers. Expected:" +
            str(len(gtJson['data'])) + " Found:" + str(len(submJson)) + ")")

    gtQuestions = sorted([r['questionId'] for r in gtJson['data']])
    res_id_to_index = {
        int(json.loads(r.strip())['questionId']): ix
        for ix, r in enumerate(submJson)
    }
    detQuestions = sorted(
        [json.loads(r.strip())['questionId'] for r in submJson])
    ...
    for gtObject in gtJson['data']:
        ...
        try:
            ...
            raise Exception("The Det file is not valid. Question " +
                            str(gtObject['questionId']) + " not present")
            ...
            if not 'answer' in detObject:
                raise Exception("Question " + str(gtObject['questionId']) +
                                " not valid (no answer key)")

            if isinstance(detObject['answer'], list) == True:
                raise Exception(
                    "Question " + str(gtObject['questionId']) +
                    " not valid (answer key has to be a single string)")
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams, num_chunks):
    ...
    if num_chunks > 1:
        submJson = []
        for _idx in range(args.num_chunks):
            file = submFilePath.replace('.json',
    ...
    if show_scores_per_answer_type:
        ...
        reasoningTypeNumQuestions = {
            x: 0 for x in reasoning_requirements.keys()
        }
    ...
    for gtObject in gtJson['data']:
        ...
        if q_id in question_ids_to_exclude:
            ...
            for answer in gtObject['answers']:
                ...
                det_answer = ' '.join(
                    detObject['answer'].strip().lower().split())

                #dist = levenshtein_distance(answer.lower(), detObject['answer'].lower())
                dist = levenshtein_distance(gt_answer, det_answer)
                length = max(len(answer.upper()),
                             len(detObject['answer'].upper()))
                values.append(0.0 if length == 0 else float(dist) /
                              float(length))
    ...
    methodMetrics = {
        'score':
            0 if len(gtJson['data']) == 0 else totalScore /
            (len(gtJson['data']) - len(question_ids_to_exclude))
    }
    ...
    if show_scores_per_answer_type:
        for a_type, ref in answer_types.items():
            answer_types_scores[ref] = 0 if len(
                gtJson['data']) == 0 else answerTypeTotalScore[a_type] / (
                    answerTypeNumQuestions[a_type])

        for e_type, ref in evidence_types.items():
            evidence_types_scores[ref] = 0 if len(
                gtJson['data']) == 0 else evidenceTypeTotalScore[e_type] / (
                    evidenceTypeNumQuestions[e_type])

        for r_type, ref in reasoning_requirements.items():
            operation_types_scores[ref] = 0 if len(
                gtJson['data']) == 0 else reasoningTypeTotalScore[r_type] / (
...
def display_results(results, show_answer_types):
    ...
    if show_answer_types:
        ...
        for a_type in answer_types.values():
            print("\t{:12s} {:2.4f}".format(
                a_type, results['scores_by_types']['answer_types'][a_type]))

        print("\nEvidence types:")
        for e_type in evidence_types.values():
            print("\t{:12s} {:2.4f}".format(
                e_type, results['scores_by_types']['evidence_types'][e_type]))

        print("\nOperation required:")
        for r_type in reasoning_requirements.values():
            print("\t{:12s} {:2.4f}".format(
                r_type, results['scores_by_types']['operation_types'][r_type]))


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="InfographVQA evaluation script.")
    ...
    parser.add_argument(
        ...
        type=str,
        help=
        "Path to a directory where to copy the file 'results.json' that contains per-sample results.",
        required=False)
    ...
    submission_file = os.path.join(args.submission_file,
                                   args.test_file).replace('json', 'jsonl')
    # Validate the format of ground truth and submission files.
    validate_data(args.ground_truth, submission_file, args.num_chunks)

    # Evaluate method
    results = evaluate_method(args.ground_truth, submission_file, args,
                              args.num_chunks)
    ...
    if args.output:
        ...
        print(
            "All results including per-sample result has been correctly saved!")
